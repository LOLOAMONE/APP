  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((o) => !o)}
        disabled={switching}
        className="flex w-full items-center justify-between gap-1 rounded-md border border-gray-300 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-50"
      >
        <span className="truncate">{networkView ? "Vue réseau" : activeRestaurant?.name ?? "Choisir un restaurant"}</span>
        <ChevronDown className="h-4 w-4 shrink-0" aria-hidden />
      </button>
      {open && (
        <div className="absolute left-0 top-full z-20 mt-1 w-full min-w-[14rem] rounded-md border border-gray-200 bg-white py-1 shadow-lg">
          {isSuperAdmin && (
            <button
